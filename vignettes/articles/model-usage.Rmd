...
Due to the total size of the `r nrow(elements::ModelledTaxaCodes)` ENMs currently included in `elements` (1.8GB when compressed, 7.5GB in memory)
...
For example, below `elements::env_filter` is applied to all taxa in `elements::ModelledTaxaCodes` using the svm method, with the predictors derived from `elements::ExampleScenarios`.
...
The ability to calculate the niche overlap between taxa is provided through the
`elements::calc_overlap` function, below the niche overlaps for the taxa present
in `elements::ExamplePlots` are displayed.

```{r calc_overlap_example}
