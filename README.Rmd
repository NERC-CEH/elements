...
<!-- badges: start -->
[![Generic badge](https://img.shields.io/badge/Version-0.5.2-green.svg)]()
...
```{r model_scenarios, include = FALSE}
scenario_results <- elements::predict_occ(taxa_codes = elements::ExamplePlot$taxon_code,
                                          predictors = elements::ExampleScenarios,
                                          append_predictors = TRUE)

scenario_c_results <- subset(x = scenario_results, scenario_code == "c")
...
legend("right", inset = c(-1.175, 0), legend = elements::ExamplePlot$taxon_name, title = "Taxon",
       cex = 1,
       fill = grDevices::palette.colors()[1:length(model_taxa_codes)], bty = "n")
