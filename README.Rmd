...
<!-- badges: start -->
[![Generic badge](https://img.shields.io/badge/Version-0.6.5-green.svg)]()
...
At present there are `r nrow(elements::TaxonomicBackbone)` ENMs currently included in `elements`, the table below displays the number of taxa by Phylum.

```{r phylum_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="The number of taxa in elements by Phylum"}
phylum_summary <- elements::TaxonomicBackbone |>
  dplyr::summarise("n" = dplyr::n(),
                   .by = phylum) |>
  tidyr::pivot_wider(names_from = phylum,
                     values_from = n)

knitr::kable(phylum_summary, format = "html")
```
